...
export const POST: APIRoute = async (ctx) => {
  ...
  try {
    ...
    console.error('Question submission initialization failed.', error);
  ...
  const outcome = await new SubmissionService(
    ...
    mc.verifier,
    mc.formRateLimiter,
  ).submit(parsed.body, { hostname: new URL(ctx.request.url).hostname, clientKey: key });
  return outcome.kind === 'ok'
    ? json({ ok: true, message: 'Thank you! Your question is pending review.' })
