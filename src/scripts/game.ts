...
import { GameEngine, SessionSeenStore, formatVotes, loadManifest, verdictText } from './game-engine';
...
export function initGame(): void {
  ...
  const textA = $('text-a'); const textB = $('text-b'); const percentA = $('percent-a'); const percentB = $('percent-b'); const votesA = $('votes-a'); const votesB = $('votes-b'); const fillA = $('fill-a'); const fillB = $('fill-b'); const verdict = $('verdict-text'); const nextButton = $<HTMLButtonElement>('next-button'); const live = $('game-live'); const shareButton = $<HTMLButtonElement>('share-button'); const fullscreenButton = $<HTMLButtonElement>('fullscreen-button'); const packButton = $<HTMLButtonElement>('pack-button'); const packLabel = $('pack-label'); const packDialog = $<HTMLDialogElement>('pack-dialog'); const packGrid = $('pack-grid'); const packPicker = $('pack-picker'); const ageGate = $('age-gate'); const ageGatePack = $('age-gate-pack');
  if (!stage) return;
  ...
  const setVotePending = (pending: boolean) => { choiceA && (choiceA.disabled = pending); choiceB && (choiceB.disabled = pending); gameStage.setAttribute('aria-busy', String(pending)); };
  ...
  function renderQuestion(question: GameQuestion): void {
    ...
    setVotePending(false); [choiceA, choiceB].forEach((button) => button?.classList.remove('picked', 'not-picked'));
    if (textA) textA.textContent = question.a; if (textB) textB.textContent = question.b;
    [percentA, percentB, votesA, votesB, verdict].forEach((element) => { if (element) element.textContent = ''; });
  ...
  function showResult(result: VoteResult, choice: 'A' | 'B'): void {
    ...
    if (percentA) percentA.textContent = `${result.percentA}%`; if (percentB) percentB.textContent = `${result.percentB}%`; if (votesA) votesA.textContent = formatVotes(result.votesA); if (votesB) votesB.textContent = formatVotes(result.votesB);
    requestAnimationFrame(() => { if (fillA) fillA.style.height = `${result.percentA}%`; if (fillB) fillB.style.height = `${result.percentB}%`; });
    const selectedPercent = choice === 'A' ? result.percentA : result.percentB; const message = verdictText(selectedPercent, result.total);
    if (verdict) verdict.textContent = message;
    announce(`${message} Option A ${result.percentA} percent, ${formatVotes(result.votesA)}. Option B ${result.percentB} percent, ${formatVotes(result.votesB)}. Total ${formatVotes(result.total)}.`);
  }
  async function vote(choice: 'A' | 'B'): Promise<void> {
    if (!current || busy) return;
    if (!config.votingEnabled) { setNotice('Voting is currently switched off — enjoy the questions!'); return; }
    busy = true; setVotePending(true);
    try {
      const response = await fetch(config.voteApi, { method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json' }, credentials: 'same-origin', body: JSON.stringify({ questionId: current.id, choice }) });
      const body = (await response.json().catch(() => null)) as { ok?: boolean; result?: VoteResult; message?: string } | null;
      if (!response.ok || !body?.ok || !body.result) throw new Error(body?.message ?? `vote ${response.status}`);
      showResult(body.result, choice);
    } catch {
      setNotice('Voting is temporarily unavailable. Please try again or keep playing.');
      announce('Voting is temporarily unavailable. Please try again or keep playing.');
    } finally { busy = false; setVotePending(false); }
  }
  ...
  choiceA?.addEventListener('click', () => void vote('A')); choiceB?.addEventListener('click', () => void vote('B')); nextButton?.addEventListener('click', () => void nextQuestion()); shareButton?.addEventListener('click', () => void share()); fullscreenButton?.addEventListener('click', toggleFullscreen); document.addEventListener('fullscreenchange', updateFullscreenLabel); document.addEventListener('webkitfullscreenchange', updateFullscreenLabel);
  ...
  document.addEventListener('keydown', (event) => { if (event.defaultPrevented || packDialog?.open || document.getElementById('nav-links')?.classList.contains('open')) return; const target = event.target as HTMLElement | null; if (target && (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA' || target.tagName === 'SELECT' || target.isContentEditable)) return; if (event.key === 'ArrowLeft') { event.preventDefault(); void vote('A'); } else if (event.key === 'ArrowRight') { event.preventDefault(); void vote('B'); } else if ((event.key === 'Enter' || event.key === ' ') && hasVoted && target?.closest('.game-shell')) { event.preventDefault(); void nextQuestion(); } else if ((event.key === 'c' || event.key === 'C') && packButton) openDialog(); else if ((event.key === 'f' || event.key === 'F') && fullscreenButton && !fullscreenButton.hidden) toggleFullscreen(); });
